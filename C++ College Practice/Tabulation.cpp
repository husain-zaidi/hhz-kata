...
void simplify()
{
	...
	vector<binaryNumber> remainingPrimeImplicants = unchecked;
	...
	for(int min: minterms){
		...
		if(primeCoveringMinterm.size() == 1){
			...
			if(flag){
				...
				remainingPrimeImplicants.erase(remainingPrimeImplicants.begin()+pos);
	...
	for(binaryNumber num: remainingPrimeImplicants){
		...
		for(int k:num.variables){
			if(k == remainingMinTerms[i]){  //if remaining prime implicant has remaining minterms
				flag = 1;
			}else{
				flag = 0;
				break;
